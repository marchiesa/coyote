 ...
 *
 */
public class MainRackeComputePerformanceCompareECMP {
	...
	public static void main(String[] args) throws IOException, GRBException, ClassNotFoundException{
		...
		for(int w=1;w<=50;w++){
			FileInputStream fileIn = new FileInputStream(file.split("\\.")[0].split("/")[2]+"-solution"+w);
			...
			//destination2ht = MainMosekFormulationGeneratorAmpl.readDestination2HtFromFile(args);
			if(destination2ht==null){
				destination2ht = MainMosekFormulationGeneratorAmpl.computeD2HTShortestPath(g,setOfDemands);
				destination2ht = MainMosekFormulationGeneratorAmpl.computeD2HTAddArcsCorrect(g,setOfDemands,destination2ht);
			}
			Map<Integer,Map<Pair<Integer>,Double>> arc2demand2fraction = MainMosekFormulationGeneratorAmpl.computeECMPArc2Demand2Fraction(g,destination2ht,setOfDemands);
			...
			System.out.println("results/"+args[0].split("\\.")[0].split("/")[2]+"-ecmp.txt");
			lpSolver.computeObliviousPerformance(g, setOfDemands,arc2demand2fraction,trafficMatrix,1);
			double ecmpOpt = lpSolver.getOpt();
			lpSolver = new CompactLPSolverObliviousPerformanceGivenDemand();
			lpSolver.computeObliviousPerformance(g, setOfDemands,solution,trafficMatrix,1);
			double rackeOpt = lpSolver.getOpt();
			//lpSolver.computeObliviousPerformance(g, solution);
			writer.println(w+" "+ecmpOpt + " " +rackeOpt);
			System.out.println(w+" "+ecmpOpt + " " +rackeOpt);			
		}
		writer.flush();
		writer.close();

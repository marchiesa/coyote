...
public class LPSolverOptimalObliviousGivenDemand {
	...
	public void computeOptimalPerformance(Graph g,List<Pair<Integer>> setOfDemands ){
		this.computeOptimalPerformance(g, setOfDemands,null,null,1);
	}
	...
	public void computeOptimalPerformance(Graph g,Map<Integer,Map<Pair<Integer>,Double>> arc2demand2fraction ){
		...
		this.computeOptimalPerformance(g, new LinkedList<Pair<Integer>>(setOfDemands),arc2demand2fraction,null,1);
	}


	public void computeOptimalPerformance(Graph g,List<Pair<Integer>> setOfDemands, Map<Integer,Map<Pair<Integer>,Double>> arc2demand2fraction, Map<Pair<Integer>,Double> demand2estimate, double w){
		...
		try {
			...
			env.set(GRB.DoubleParam.BarConvTol, 0.1d);
			...
			for(Arc arc:g.getArcs()){
				...
				for(Pair<Integer> demand : setOfDemands){
					...
					vertex2var.put(demand.getSecond(), model.addVar(0,GRB.INFINITY, 0, GRB.CONTINUOUS,"splus_"+arc.toStringShort()+"^("+demand.getFirst()+","+demand.getSecond()+")"));
					vertex2var2.put(demand.getSecond(), model.addVar(0,GRB.INFINITY, 0, GRB.CONTINUOUS,"sminus_"+arc.toStringShort()+"^("+demand.getFirst()+","+demand.getSecond()+")"));
					//System.out.print("pi_"+arc.toStringShort()+"^("+i+","+j+")"+" ");
			...
			for(Arc arcE: g.getArcs()){
				...
				Map<Integer,Map<Integer,GRBVar>> vertex2vertex2var2 = this.splus_edge2vertex2vertex2var.get(arcE.getId());
				Map<Integer,Map<Integer,GRBVar>> vertex2vertex2var3 = this.sminus_edge2vertex2vertex2var.get(arcE.getId());
				for(Pair<Integer> demand: setOfDemands){
					if((arc2demand2fraction!=null && arc2demand2fraction.get(arcE.getId())!=null && arc2demand2fraction.get(arcE.getId()).get(demand)!=null ) ||
						...
						Map<Integer,GRBVar> vertex2var = vertex2vertex2var.get(demand.getFirst());
						Map<Integer,GRBVar> vertex2var2 = vertex2vertex2var2.get(demand.getFirst());
						Map<Integer,GRBVar> vertex2var3 = vertex2vertex2var3.get(demand.getFirst());
						GRBLinExpr flows = new GRBLinExpr();
						...
						flows.addTerm(1/arcE.getCapacity(), this.x_edge2demand2var.get(arcE.getId()).get(new Pair<Integer>(demand.getFirst(),demand.getSecond())));
						flows.addTerm(-1, vertex2var2.get(demand.getSecond()));
						flows.addTerm(1, vertex2var3.get(demand.getSecond()));
			...
			for(Arc arcE: g.getArcs()){
				...
				for(Pair<Integer> demand: setOfDemands){
					...
					//System.out.print(" + "+vertex2var.get(demand.getSecond()).get(GRB.StringAttr.VarName));
					//System.out.print(" - "+this.x_edge2demand2var.get(arcE.getId()).get(new Pair<Integer>(demand.getFirst(),demand.getSecond())).get(GRB.StringAttr.VarName));
					if(demand2estimate==null){
						flows.addTerm(-1, vertex2var3.get(demand.getSecond()));
					}else{
						flows.addTerm(demand2estimate.get(demand)/w, vertex2var2.get(demand.getSecond()));
						flows.addTerm(-demand2estimate.get(demand)*w, vertex2var3.get(demand.getSecond()));
					}

 ...
 *
 */
public class CompactLPSolverObliviousPerformance {
	...
	public void computeObliviousPerformance(Graph g, Map<Integer,Map<Pair<Integer>,Double>> arc2demand2fraction){
		this.computeObliviousPerformance(g, arc2demand2fraction,false);
	}

	public void computeObliviousPerformance(Graph g, Map<Integer,Map<Pair<Integer>,Double>> arc2demand2fraction, boolean AGGREGATE_CONGESTION){

		this.arc2rLoad= new HashMap<Integer,Double>();

		Set<Integer> destinations = new TreeSet<Integer>();
		for(Integer arcId: arc2demand2fraction.keySet())
			for(Pair<Integer> demand : arc2demand2fraction.get(arcId).keySet())
				destinations.add(demand.getSecond());
		...
		try {
			...
			Set<Integer> analyzedArcs4 = new TreeSet<Integer>();
			for(Integer idArc : arc2demand2fraction.keySet()){
				if(AGGREGATE_CONGESTION && analyzedArcs4.contains(2*(idArc/2)))
					continue;
				analyzedArcs4.add(2*(idArc/2));
				...
				if(AGGREGATE_CONGESTION && demand2fractionBReversed!=null)
				...
				for(Pair<Integer> demand: demandsOfInterest){
					...
					if(AGGREGATE_CONGESTION && cost==0 && demand2fractionBReversed!=null){
				...
				for(Vertex source: g.getVertices()){
					for(Vertex destination: g.getVertices()){
						...
						//System.out.print("\t");
						for(Arc outgoingArc : destination.getArcs()){
							flows.addTerm(1, arc2idVertex2var.get(outgoingArc.getId()).get(source.getId()));
							...
							if(print)System.out.print("+ "+arc2idVertex2var.get(outgoingArc.getId()).get(source.getId()).get(GRB.StringAttr.VarName)+
				...
				for(Arc arc2 : g.getArcs()){
					...
					//System.out.println(" <= "+arc2.getCapacity());

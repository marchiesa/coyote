...
/** THIS IS NOT FASTER!! USE COMPACT!! **/
public class FasterCompactLPSolverObliviousPerformance {
	...
	private GRBEnv env;
	private Map<Pair<Integer>,GRBVar> demand2var;
	private Map<Integer,Map<Integer,GRBVar>> arc2idVertex2var;
	
	public FasterCompactLPSolverObliviousPerformance() throws GRBException{
		env =new GRBEnv();
		env.set(GRB.IntParam.OutputFlag,0);
		model = new GRBModel(env);
		model.set(GRB.StringAttr.ModelName, "maximize_edge");
		demand2var  = new HashMap<Pair<Integer>,GRBVar>();
		arc2idVertex2var = new HashMap<Integer,Map<Integer,GRBVar>>();
	}
				...
				model.addConstr(flows, GRB.EQUAL, 1,null);*/
					...
					for(Vertex vertex: g.getVertices()){
						for(Vertex vertex2: g.getVertices()){
							...
							if(cost>0)
								obj.addTerm(cost, demand2var.get(demand));
								demand2var.put(demand,model.addVar(0,GRB.INFINITY, (cost)/arc.getCapacity(), GRB.CONTINUOUS,getString(demand)));
						}
					}
				}

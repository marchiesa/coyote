...
public class MosekFormulationGeneratorAmpl implements IMosekFormulationGeneratorAmpl {
	...
	public MosekFormulationGeneratorAmpl(){
	...
	public void computeOptimalPerformance(String graphName, Graph g,
		...
		this.computeOptimalPerformance(graphName, g, setOfDemands, destination2ht, false);
	}
	...
	public void computeOptimalPerformance(String graphName, Graph g,
		...
		try {
			...
			for(Arc arc:g.getArcs()){
				Set<Integer> analyzedArcs3 = new TreeSet<Integer>();
				if(aggregateCongestion && (arc.getId() % 2 == 1))
					continue;
				...
				for(Arc arc2:getAllArcs(destination2ht)){
					...
					if(aggregateCongestion && analyzedArcs3.contains(2*(arc2.getId()/2)))
						continue;
					analyzedArcs3.add(2*(arc2.getId()/2));
					edge2index.put(2*(arc2.getId()/2),indexCounter++);
					blx.add(Integer.MIN_VALUE);
					//		System.out.println("blx = [-inf];");
					//System.out.println("var w_"+arc.getId()+"_"+arc2.getId()+";");
					writer.println("var w_"+arc.getId()+"_"+2*(arc2.getId()/2)+";");
					//System.out.print("w_"+arc.toStringShort()+"^("+arc2.toStringShort()+" ");
				}
				//System.out.println();
			}
			PrintWriter writerAmpl = new PrintWriter(graphName+"-signomial.run", "UTF-8");
			writerAmpl.println("model "+graphName+"-formulation2.mod");
			writerAmpl.println("option solver mosek;");
			//writerAmpl.println("option mosek_options 'MSK_IPAR_OPTIMIZER=MSK_OPTIMIZER_INTPNT MSK_DPAR_INTPNT_NL_TOL_DFEAS=0.01 MSK_DPAR_INTPNT_NL_TOL_PFEAS=0.01 MSK_DPAR_INTPNT_NL_TOL_REL_GAP=0.01';");
			//MSK_IPAR_LOG_INTPNT
	...
	/*		writerAmpl.print("display ");
			//CREATE CONG VARIABLES w_e(a)

			for(Arc arc:g.getArcs()){
				//		System.out.println("blx = [-inf];");
				//System.out.println("var w_"+arc.getId()+"_"+arc2.getId()+";");
				writer.println("var cong_"+arc.getId()+";");
				writerAmpl.print(" cong_"+arc.getId());
				//System.out.print("w_"+arc.toStringShort()+"^("+arc2.toStringShort()+" ");
				//System.out.println();
			}
			writerAmpl.println(";");*/
			...
			for(Integer destId : destinationVertices){
				...
				//for(Vertex v: shortestPathDestination2ht.get(destId).getVertices()){
				for(Vertex v: destination2ht.get(destId).getVertices()){
					if(v.getId()!=destId){
			...
			for(Arc arc: g.getArcs()){
				...
				for(Integer destId : destinationVertices){
					...
					Set<Integer> sourcesInSE = this.buildSE(arc.getId(), destination2ht.get(destId), getSourcesToDestination(setOfDemands,destId),g);
					if(aggregateCongestion){
						sourcesInSE.addAll(this.buildSE(g.getReversedArc(arc.getId()).getId(), destination2ht.get(destId), getSourcesToDestination(setOfDemands,destId),g)); 
					}
					for(Integer sourceId : sourcesInSE){
						for(Integer arcId2 : this.buildHTS(sourceId, destination2ht.get(destId)).getSecond()){
							analyzedArcs.add(2*(arcId2/2));
			...
			//fifth type of constraint if you use cong variables
			/*for(Arc arc: g.getArcs()){
				//fifthConstraintArc2index.put(arc.getId(), indexConstraint);
				//System.out.print("subject to dualobjective_"+arc.getId()+": alpha ");
				writer.print("subject to dualobjective_"+arc.getId()+": cong_"+arc.getId());
				//				entriesInMatrixA.add(new Triplet<Integer,Double>(indexConstraint,alphaIndex,1d));
				for(Integer arcId2:arcId2arcId.get(arc.getId())){
					Arc arc2 = g.getArcById(arcId2);
					//System.out.print( " - "+ arc2.getCapacity() + " * w_"+arc.getId()+"_"+arc2.getId() + " ");
			...
			for(Arc arc:g.getArcs()){
				//		System.out.println("blx = [-inf];");
				//System.out.println("var w_"+arc.getId()+"_"+arc2.getId()+";");
				writer.print("subject to dualedgeload_"+arc.getId()+": -cong_"+arc.getId()+" alpha ");
				writer.println(" >= 0;");
			}*/
			...
			for(Arc arc: g.getArcs()){
				...
				for(Integer destId : destinationVertices){
					...
					Set<Integer> sourcesInSE = this.buildSE(arc.getId(), destination2ht.get(destId), this.getSourcesToDestination(setOfDemands, destId),g);
					Set<Integer> sourcesInSEReversed = this.buildSE(g.getReversedArc(arc.getId()).getId(), destination2ht.get(destId), this.getSourcesToDestination(setOfDemands, destId),g);
					Set<Integer> sourcesInSEUnion = new TreeSet<Integer>(sourcesInSE);
					if(aggregateCongestion)
							sourcesInSEUnion.addAll(sourcesInSEReversed);
					for(Integer sourceId:sourcesInSEUnion){
						...
						writer.print( arc.getCapacity() + " * pi_"+arc.getId()+"_"+destId + "_"+sourceId+"  ");
						if(sourcesInSE.contains(sourceId)){
						...
						if(aggregateCongestion && sourcesInSEReversed.contains(sourceId)){
							//System.out.print(" - exp( f_"+sourceId+"_"+destId + "_" + arc.getSecondEndPoint().getId()+" ");
							writer.print(" - exp( f_"+sourceId+"_"+destId + "_" + arc.getSecondEndPoint().getId()+" ");
							//System.out.print(" + phi_"+destId+"_"+arc.getSecondEndPoint().getId()+"_"+g.getReversedArc(arc.getId()).getId() + " ) ");
							writer.print(" + phi_"+destId+"_"+arc.getSecondEndPoint().getId()+"_"+g.getReversedArc(arc.getId()).getId() + " ) ");
						}
						//System.out.println(" >= 0;");
						writer.println(" >= 0;");
	...
	public void computeOptimalPerformance(String graphName, Graph g,
		...
		this.computeOptimalPerformance(graphName, g, setOfDemands, destination2ht, demand2estimate, w, false);
	...
	public void computeOptimalPerformance(String graphName, Graph g,
		...
		this.computeOptimalPerformance(graphName, g, setOfDemands, destination2ht, demand2estimate, 1000, false);		
	}
}
...
public class MosekFormulationGeneratorAmplHose implements IMosekFormulationGeneratorAmpl {
	...
	private Map<Integer,Map<Integer,Integer>>  w_edge2vertex2index;
	...
	public MosekFormulationGeneratorAmplHose(){
	...
	public void computeOptimalPerformance(String graphName, Graph g,
		...
		try {
			...
			Map<Integer,Double> idVertex2cumulativeCapacity = new HashMap<Integer,Double>();
			...
			for(Vertex vertex: g.getVertices()){
				...
				for(Arc arc : vertex.getArcs())
					sum+=arc.getCapacity();
				idVertex2cumulativeCapacity.put(vertex.getId(), sum);
			}
			...
			// CREATE W VARIABLES w_e(v)
			this.w_edge2vertex2index = new HashMap<Integer,Map<Integer,Integer>>();

			for(Arc arc:g.getArcs()){
				if(arc.getId() % 2 ==1)
					continue;
				Map<Integer,Integer> vertex2index = new HashMap<Integer,Integer>();
				this.w_edge2vertex2index.put(arc.getId(), vertex2index); 
				for(Vertex v:g.getVertices()){
					vertex2index.put(v.getId(),indexCounter++);
					blx.add(Integer.MIN_VALUE);
					//		System.out.println("blx = [-inf];");
					//System.out.println("var w_"+arc.getId()+"_"+arc2.getId()+";");
					writer.println("var w_"+arc.getId()+"_"+v.getId()+";");
	...
	/*		writerAmpl.print("display ");
			...
			for(Arc arc: g.getArcs()){
				...
				for(Vertex vertex:g.getVertices()){
					...
					writer.print( " - "+ idVertex2cumulativeCapacity.get(vertex.getId()) + " * w_"+arc.getId()+"_"+vertex.getId() + " ");
			...
			for(Pair<Integer> demand : setOfDemands){
				...
				for(Integer idArc: hts.getSecond()){
					int idArcAnalyzed = 2*(idArc/2);
					Arc arc = g.getArcById(idArcAnalyzed);
					writer.print("subject to dualdemandpotential_"+arc.getId()+"_("+demand.getFirst()+"_"+demand.getSecond()+"):  ");
					//System.out.print( arc.getCapacity() + " * pi_"+arc.getId()+"_"+destId + "_"+sourceId+"  ");
					writer.print( arc.getCapacity() + " * w_"+arc.getId()+"_"+demand.getFirst()+" ");
					writer.print( arc.getCapacity() + " * w_"+arc.getId()+"_"+demand.getSecond()+"  ");
					//System.out.print(" - exp( f_"+sourceId+"_"+destId + "_" + arc.getFirstEndPoint().getId()+" ");
					writer.print(" - exp( f_"+demand.getFirst()+"_"+demand.getSecond() + "_" + arc.getFirstEndPoint().getId()+" ");
					//System.out.print(" + phi_"+destId+"_"+arc.getFirstEndPoint().getId()+"_"+arc.getId() + " ) ");
					writer.print(" + phi_"+demand.getSecond()+"_"+arc.getFirstEndPoint().getId()+"_"+arc.getId() + " ) ");
					writer.print(" - exp( f_"+demand.getFirst()+"_"+demand.getSecond() + "_" + arc.getSecondEndPoint().getId()+" ");
					//System.out.print(" + phi_"+destId+"_"+arc.getFirstEndPoint().getId()+"_"+arc.getId() + " ) ");
					writer.print(" + phi_"+demand.getSecond()+"_"+arc.getSecondEndPoint().getId()+"_"+g.getReversedArc(arc.getId()).getId() + " ) ");
					//System.out.println(" >= 0;");
					writer.println(" >= 0;");
				}
			}

			for(Integer arcId  : this.w_edge2vertex2index.keySet()){
				for(Vertex vertex:g.getVertices()){
					//System.out.println("subject to variablebound_w_"+arcId+"_"+arcId2+": w_"+arcId+"_"+arcId2+" >=0;");
					writer.println("subject to variablebound_w_"+arcId+"_"+vertex.getId()+": w_"+arcId+"_"+vertex.getId()+" >=0;");
				}
			}

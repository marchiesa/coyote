...
public class MosekFormulationGeneratorAmplGivenDemand implements IMosekFormulationGeneratorAmpl {
	...
	private Map<Integer,Map<Integer,Map<Integer,GRBVar>>> splus_edge2vertex2vertex2var;
	private Map<Integer,Map<Integer,Map<Integer,GRBVar>>> sminus_edge2vertex2vertex2var;
	...
	public MosekFormulationGeneratorAmplGivenDemand(){
	...
	public void computeOptimalPerformance(String graphName, Graph g,
			...
			Map<Integer,Graph> destination2ht , Map<Pair<Integer>,Double> demand2estimate, double w){
		this.computeOptimalPerformance(graphName, g, setOfDemands, destination2ht, demand2estimate, destination2ht,w,false);
	}

	public void computeOptimalPerformance(String graphName, Graph g,
			List<Pair<Integer>> setOfDemands,
			Map<Integer,Graph> destination2ht , Map<Pair<Integer>,Double> demand2estimate, 
			Map<Integer,Graph> destination2htOptimal, double w,boolean aggregateCongestion){

		try {
			//System.out.println(g);
			PrintWriter writer = new PrintWriter(graphName+"-formulation.mod", "UTF-8");
			writer.println("option auxfiles rc;");
			...
			for(Arc arc:g.getArcs()){
				...
				for(Integer destId: destinationVertices){
					...
					for(Vertex j: destination2htOptimal.get(destId).getVertices()){
						if(destId==j.getId())
							continue;
						vertex2index.put(j.getId(),indexCounter++);
						blx.add(Integer.MIN_VALUE);
						//System.out.println("var pi_"+arc.getId()+"_"+destId+"_"+j.getId()+";");
						writer.println("var pi_"+arc.getId()+"_"+destId+"_"+j.getId()+";");
						if(!first){
							writerAmpl.print(",");
						}else
							first=false;
						writerAmpl.print(" pi_"+arc.getId()+"_"+destId+"_"+j.getId());
			...
			for(Arc arc:g.getArcs()){
				...
				for(Arc arc2:getAllArcs(destination2htOptimal)){
					...
					writer.println("var w_"+arc.getId()+"_"+arc2.getId()+";");
					if(!first){
						writerAmpl.print(",");
					}else
						first=false;
					writerAmpl.print(" w_"+arc.getId()+"_"+arc2.getId());
			...
			// CREATE S+ S- VARIABLES s+_e(i,j) and s-_e(i,j)
			this.splus_edge2vertex2vertex2var = new HashMap<Integer,Map<Integer,Map<Integer,GRBVar>>>();
			this.sminus_edge2vertex2vertex2var = new HashMap<Integer,Map<Integer,Map<Integer,GRBVar>>>();
			...
			for(Arc arc:g.getArcs()){
				...
				splus_edge2vertex2vertex2var.put(arc.getId(), vertex2vertex2var);
				Map<Integer,Map<Integer,GRBVar>> vertex2vertex2var2 = new HashMap<Integer,Map<Integer,GRBVar>>();
				sminus_edge2vertex2vertex2var.put(arc.getId(), vertex2vertex2var2);

				for(Pair<Integer> demand : setOfDemands){
					Map<Integer,GRBVar> vertex2var = vertex2vertex2var.get(demand.getFirst());
					Map<Integer,GRBVar> vertex2var2 = vertex2vertex2var2.get(demand.getFirst());
					if(vertex2var == null){
						vertex2var = new HashMap<Integer,GRBVar>();
						vertex2vertex2var.put(demand.getFirst(), vertex2var);
						vertex2var2 = new HashMap<Integer,GRBVar>();
						vertex2vertex2var2.put(demand.getFirst(), vertex2var2);
					}

					writer.println("var splus_"+arc.getId()+"_"+demand.getFirst()+"_"+demand.getSecond()+";");
					writer.println("var sminus_"+arc.getId()+"_"+demand.getFirst()+"_"+demand.getSecond()+";");
					/*if(!first){
						writerAmpl.print(",");
					}else
						first=false;
					writerAmpl.print("  splus_"+arc.getId()+"_"+demand.getFirst()+"_"+demand.getSecond()+", "
							+ "sminus_"+arc.getId()+"_"+demand.getFirst()+"_"+demand.getSecond());*/
					//System.out.print("pi_"+arc.toStringShort()+"^("+i+","+j+")"+" ");
			...
			PrintWriter writerSplittingRatio = new PrintWriter(graphName+"-splitting-ratio-formulation-data.txt", "UTF-8");
			...
			Map<Integer,Set<Integer>> arcId2arcId = new HashMap<Integer,Set<Integer>>();
			for(Arc arc: g.getArcs()){
				if(aggregateCongestion && (arc.getId() % 2 == 1))
					continue;
				Map<Integer,Integer> edge2index = new HashMap<Integer,Integer>();
				//seventhConstraintEdge2edge2index.put(arc.getId(), edge2index);
				Set<Integer> analyzedArcs = new TreeSet<Integer>();
				arcId2arcId.put(arc.getId(), analyzedArcs);
				for(Integer destId : destinationVertices){
					Set<Integer> analyzedArcs2 = new TreeSet<Integer>();

					//get the selected source vertices
					//for each arc in hs(t) - do I have it?
		
					
					//for(Integer sourceId : this.buildSE(arc.getId(), destination2htOptimal.get(destId), getSourcesToDestination(setOfDemands,destId),g)){
					for(Integer sourceId : sourceVertices){
						for(Integer arcId2 : this.buildHTS(sourceId, destination2htOptimal.get(destId)).getSecond()){
							analyzedArcs.add(arcId2);
							if(analyzedArcs2.contains(arcId2))
								continue;
							analyzedArcs2.add(arcId2);
							//System.out.print("subject to dualpotential_"+arc.getId()+"_"+destId+"_"+arcId2+":  ");
							writer.print("subject to dualpotential_"+arc.getId()+"_"+destId+"_"+arcId2+":  ");
							
							if(aggregateCongestion){
								//System.out.print(" + w_"+arc.getId()+"_"+2*(arcId2/2)+ " ");
								writer.print(" + w_"+arc.getId()+"_"+2*(arcId2/2)+ " ");
							}
							else{
								//System.out.print(" + w_"+arc.getId()+"_"+2*(arcId2/2)+ " ");
								writer.print(" + w_"+arc.getId()+"_"+arcId2+ " ");
							}
							
							Arc arc2 = g.getArcById(arcId2);
							if(arc2.getSecondEndPoint().getId()!=destId){
								//System.out.print(" + pi_"+arc.getId()+"_"+destId+"_"+arc2.getSecondEndPoint().getId() + " ");
								writer.print(" + pi_"+arc.getId()+"_"+destId+"_"+arc2.getSecondEndPoint().getId() + " ");
							}
							//System.out.print(" - pi_"+arc.getId()+"_"+destId+"_"+arc2.getFirstEndPoint().getId() + " ");
							writer.print(" - pi_"+arc.getId()+"_"+destId+"_"+arc2.getFirstEndPoint().getId() + " ");
							//System.out.println(" >= 0;");
							writer.println(" >= 0;");
						}
					}					
			...
			for(Arc arc: g.getArcs()){
				...
				writer.print("subject to dualVerticesobjective_"+arc.getId()+": alpha ");
				//				entriesInMatrixA.add(new Triplet<Integer,Double>(indexConstraint,alphaIndex,1d));
				for(Integer arcId2:arcId2arcId.get(arc.getId())){
					if(aggregateCongestion && (arcId2 % 2 == 1))
						continue;
					Arc arc2 = g.getArcById(arcId2);
					//System.out.print( " - "+ arc2.getCapacity() + " * w_"+arc.getId()+"_"+arc2.getId() + " ");
					writer.print( " - "+ arc2.getCapacity() + " * w_"+arc.getId()+"_"+arc2.getId() + " ");
			...
			Set<Triplet<Integer,Integer>> setOfTriplet = new TreeSet<Triplet<Integer,Integer>>();
			for(Arc arc: g.getArcs()){
				if(aggregateCongestion && (arc.getId() % 2 == 1))
					continue;
				...
				for(Pair<Integer> demand: setOfDemands){
					...
					//Set<Integer> sourcesInSE = this.buildSE(arc.getId(), destination2htOptimal.get(destId), this.getSourcesToDestination(setOfDemands, destId),g);
					//for(Integer sourceId:sourcesInSE){
					
					//for(Integer sourceId:sourceVertices){
					//System.out.print("subject to dualdemandpotential_"+arc.getId()+"_"+destId+"_"+sourceId+":  ");
					writer.print("subject to dualdemandpotential_"+arc.getId()+"_"+destId+"_"+sourceId+":  ");
					...
					if(this.f_demand2vertex2index.get(demand).get(arc.getFirstEndPoint().getId()) == null ||
							this.phi_destination2vertex2edge2index.get(destId).get(arc.getFirstEndPoint().getId()) == null ||
							this.phi_destination2vertex2edge2index.get(destId).get(arc.getFirstEndPoint().getId()).get(arc.getId()) == null)
						writer.print(" 0 ");
					else{
						writer.print(" exp( f_"+sourceId+"_"+destId + "_" + arc.getFirstEndPoint().getId()+" ");
					//System.out.print(" + phi_"+destId+"_"+arc.getFirstEndPoint().getId()+"_"+arc.getId() + " ) ");
						...
						writer.print("/" +arc.getCapacity()+" ");
					}
					if(aggregateCongestion){
						Arc reversedArc = g.getReversedArc(arc.getId());
						if(this.f_demand2vertex2index.get(demand).get(arc.getSecondEndPoint().getId()) == null ||
							this.phi_destination2vertex2edge2index.get(destId).get(arc.getSecondEndPoint().getId()) == null ||
							this.phi_destination2vertex2edge2index.get(destId).get(arc.getSecondEndPoint().getId()).get(reversedArc.getId()) == null)
						writer.print(" + 0 ");
						else{
							writer.print(" + exp( f_"+sourceId+"_"+destId + "_" + arc.getSecondEndPoint().getId()+" ");
						//System.out.print(" + phi_"+destId+"_"+arc.getFirstEndPoint().getId()+"_"+arc.getId() + " ) ");
							writer.print(" + phi_"+destId+"_"+arc.getSecondEndPoint().getId()+"_"+reversedArc.getId() + " )");
							writer.print("/" +arc.getCapacity()+" ");
						}
					}
					//System.out.println(" >= 0;");
					writer.print(" <= pi_"+arc.getId()+"_"+destId + "_"+sourceId+" ");
					writer.print(" + splus_"+arc.getId()+"_"+destId + "_"+sourceId+" ");
					writer.println(" - sminus_"+arc.getId()+"_"+destId + "_"+sourceId+";");
					setOfTriplet.add(new Triplet<Integer,Integer>(arc.getId(),destId,sourceId));
			...
			for(Arc arcE: g.getArcs()){
				if(aggregateCongestion && (arcE.getId() % 2 == 1))
					continue;
				//System.out.println("e:"+arcE.toStringShort()+" ");
				writer.print("subject to givendemand"+arcE.getId()+":  ");
				for(Pair<Integer> demand: setOfDemands){
					Integer sourceId = demand.getFirst();
					Integer destId = demand.getSecond();
					//System.out.println("demand: " + demand);
					if(setOfTriplet.contains(new Triplet<Integer,Integer>(arcE.getId(),destId,sourceId))){
						writer.print(" + sminus_"+arcE.getId()+"_"+destId + "_"+sourceId+"*"+demand2estimate.get(demand)/w+ " ");
						writer.print(" - splus_"+arcE.getId()+"_"+destId + "_"+sourceId+"*"+demand2estimate.get(demand)*w+ " ");
					}
			...
			for(Integer arcId  : this.pi_edge2vertex2vertex2index.keySet()){
				if(aggregateCongestion && (arcId % 2 == 1))
					continue;
				for(Integer destId :  this.pi_edge2vertex2vertex2index.get(arcId).keySet()){
					for(Integer vertexId : this.pi_edge2vertex2vertex2index.get(arcId).get(destId).keySet()){
						//System.out.println("subject to variablebound_pi_"+arcId+"_"+destId+"_"+vertexId+": pi_"+arcId+"_"+destId+"_"+vertexId+" >=0;");
						writer.println("subject to variablebound_pi_"+arcId+"_"+destId+"_"+vertexId+": pi_"+arcId+"_"+destId+"_"+vertexId+" >=0;");
					}
				}
			}

			for(Integer arcId  : this.w_edge2edge2index.keySet()){
				if(aggregateCongestion && (arcId % 2 == 1))
					continue;
				for(Integer arcId2 :  this.w_edge2edge2index.get(arcId).keySet()){
					if(aggregateCongestion && (arcId2 % 2 == 1))
						continue;
					//System.out.println("subject to variablebound_w_"+arcId+"_"+arcId2+": w_"+arcId+"_"+arcId2+" >=0;");
					writer.println("subject to variablebound_w_"+arcId+"_"+arcId2+": w_"+arcId+"_"+arcId2+" >=0;");
				}
			}
			...
			for(Arc arc:g.getArcs()){
				if(aggregateCongestion && (arc.getId() % 2 == 1))
					continue;
				for(Pair<Integer> demand : setOfDemands){
					if(setOfTriplet.contains(new Triplet<Integer,Integer>(arc.getId(),demand.getSecond(),demand.getFirst()))){
						writer.println("subject to variablebound_splus_"+arc.getId()+"_"+demand.getFirst()+"_"+demand.getSecond()+": splus_"+arc.getId()+"_"+demand.getFirst()+"_"+demand.getSecond()+" >=0;");
						writer.println("subject to variablebound_sminus_"+arc.getId()+"_"+demand.getFirst()+"_"+demand.getSecond()+": sminus_"+arc.getId()+"_"+demand.getFirst()+"_"+demand.getSecond()+" >=0;");
					}
					//System.out.print("pi_"+arc.toStringShort()+"^("+i+","+j+")"+" ");
				}
			}
	...
	@Override
	public void computeOptimalPerformance(String graphName, Graph g,
			List<Pair<Integer>> setOfDemands, Map<Integer, Graph> destination2ht) {
	...
	public void computeOptimalPerformance(String graphName, Graph g,
		...
		this.computeOptimalPerformance(graphName, g, setOfDemands, destination2ht, demand2estimate, destination2ht, w, aggregateCongestion);
	}

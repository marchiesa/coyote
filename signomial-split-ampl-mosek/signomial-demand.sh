...
do	
	echo $x $j
	java -jar signomial-formulation.jar -demand topologies/Backbone/$x $j --augment --aggregate
	...
	do
		...
		rm $x.ready
		mosek $x.nl -AMPL outlev=10 #MSK_IPAR_OPTIMIZER=MSK_OPTIMIZER_INTPNT MSK_DPAR_INTPNT_NL_TOL_DFEAS=0.01 MSK_DPAR_INTPNT_NL_TOL_PFEAS=0.01 MSK_DPAR_INTPNT_NL_TOL_REL_GAP=0.01 #solve $x.nl
		mv $x.nl nl/$x-$j.nl
